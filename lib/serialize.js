...
export function Client({ hisoka, store }) {
    ...
    async value(PATH, saveToFile = false) {
        try {
            ...
            const data = isBuffer ? PATH : isArrayBuffer ? Buffer.from(PATH) : 
                /^https?:\/\//.test(PATH) ? await (res = await fetch(PATH)).buffer() : 
                fs.existsSync(PATH) ? fs.readFileSync(PATH) : null;

            if (!data) throw new Error('Invalid file path or URL');

            const fileType = await fileTypeFromBuffer(data);
            filename = filename || `file.${fileType.ext}`;

            if (saveToFile) {
                await fs.promises.writeFile(filename, data);
            }
            return { data, filename, mime: fileType.mime, ext: fileType.ext };
        } catch (error) {
            console.error(error);
            throw error;
        }
    ...
    async value(jid, path, filename = "", caption = "", quoted, ptt = false, options = {}) {
        try {
            ...
            let mtype = /image/.test(mime) ? 'image' : /video/.test(mime) ? 'video' : 'document';
            const message = {
            ...
            return await hisoka.sendMessage(jid, message, { quoted });
        } catch (error) {
            console.error(error);
        }
    },
